...
class EditorPanel:
    ...
    def _create_widgets(self):
        ...
        code_label = ttk.Label(self.frame, text="Editor de Código", style='Title.TLabel')
        ...
        # Configuración inicial del optimizado (Blanco para el texto normal)
        self.optimized_editor.config(state='disabled', foreground='#FFFFFF')
    ...
    def _create_editor_tab(self, parent_frame, editable=True) -> (scrolledtext.ScrolledText, tk.Text):
        ...
        text_editor = scrolledtext.ScrolledText(
            ...
            font=('Consolas', 11),
            background=self.colors['bg_editor'],
            # Si es editable usa el color del tema, si no, usa blanco brillante
            foreground=self.colors['fg_text'] if editable else '#FFFFFF',
        ...
        text_editor.tag_configure('optimized_out',
                                  foreground='#FFAAAA', # Rosa pastel/Salmón
                                  font=('Consolas', 10, 'overstrike')) # Tachado
    ...
    def bind_events(self, on_change_callback, on_scroll_callback):
        ...
        editor.bind('<Return>', lambda e: (on_change_callback(e), 'break'))
        editor.bind('<Configure>', lambda e: self.update_line_numbers())
